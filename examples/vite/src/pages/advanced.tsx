...
export default function AdvancedPage() {
  ...
  const handleSet = async () => {
    ...
    await conn.set(key, `Created at ${new Date().toLocaleTimeString()}`, { ex: 60 });
    setLastKey(key);
  ...
  const handleGet = async () => {
    if (!lastKey) return;
    const conn = await redis.connection();
    const value = await conn.get<string>(lastKey);
  ...
  const handleMset = async () => {
    ...
    const values = await conn.mget<string>('adv:a', 'adv:b', 'adv:c');
    setManyResult({ 'adv:a': values[0], 'adv:b': values[1], 'adv:c': values[2] });
  };
          ...
          <div className="space-y-4">
            ...
            <Card className="p-6">
              <div className="flex flex-wrap gap-2 mb-4">
                <Button size="sm" onPress={handleSet}>set()</Button>
                <Button isDisabled={!lastKey} size="sm" variant="secondary" onPress={handleGet}>get()</Button>
                <Button isDisabled={!lastKey} size="sm" variant="secondary" onPress={handleExists}>exists()</Button>
                <Button isDisabled={!lastKey} size="sm" variant="secondary" onPress={handleTtl}>ttl()</Button>
                <Button isDisabled={!lastKey} size="sm" variant="outline" onPress={handleDel}>del()</Button>
                <Button size="sm" variant="danger" onPress={handleFlush}>flushdb()</Button>
              </div>

              {lastKey && (
                <div className="p-3 bg-default-100 rounded text-xs space-y-1">
                  <p><span className="font-semibold">key:</span> {lastKey}</p>
                  {getValue !== undefined && (
                    <p><span className="font-semibold">get():</span> {getValue}</p>
                  )}
                  {existsResult !== null && (
                    <p><span className="font-semibold">exists():</span> {existsResult ? 'true' : 'false'}</p>
                  )}
                  {ttlResult !== null && (
                    <p><span className="font-semibold">ttl():</span> {ttlResult}s</p>
                  )}
                </div>
              )}
            </Card>
          ...
          <div className="space-y-4">
            ...
            <Card className="p-6">
              <div className="flex items-center gap-3">
                <Button size="sm" onPress={handleDecr}>decr()</Button>
                <Chip color="accent" size="lg" variant="soft">
                  <Chip.Label>{counter ?? 0}</Chip.Label>
                </Chip>
                <Button size="sm" onPress={handleIncr}>incr()</Button>
              </div>
            </Card>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card className="p-6">
                ...
                <p className="text-default-500 text-xs mb-3">Bulk store and retrieve multiple keys</p>
                <Button size="sm" onPress={handleMset}>Run</Button>
                {manyResult && (
              ...
              <Card className="p-6">
                ...
                <p className="text-default-500 text-xs mb-3">Batch multiple commands in one request</p>
                <Button size="sm" onPress={handlePipeline}>Run</Button>
                {pipelineResult && (
                  <div className="mt-3 p-2 bg-default-100 rounded text-xs space-y-1">
                    {pipelineResult.map((r, i) => (
                      <p key={i}><span className="font-semibold">result[{i}]:</span> {String(r)}</p>
                    ))}
                  </div>
                )}
              </Card>
