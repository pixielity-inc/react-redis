...
export default function IndexPage() {
  ...
  const handleSet = async () => {
    ...
    await conn.set('demo:greeting', `Hello from Redis at ${new Date().toLocaleTimeString()}`, { ex: 30 });
    const value = await conn.get<string>('demo:greeting');
      ...
      <section className="flex flex-col items-center justify-center gap-4 py-8 md:py-10">
        ...
        <div className="flex gap-3">
          ...
          <Button
            size="lg"
            variant="outline"
            onPress={() => window.open('https://github.com/abdokouta/redis', '_blank')}
          >
            <GithubIcon size={20} />
            GitHub
          </Button>
